...
def render_finder_stamp(
    ...
    *,
    ...
    if stamp is not None:
        ...
        if matches:
            groups = group_matches(matches, dec)
            for m in matches:
...
def group_matches(matches, center_dec, tol_arcsec=0.4):
    """
...
class PlotNuclearFilterLightcurves(AbsPhotoT3Unit, AbsTabulatedT2Unit):
    ...
    lsst_archive_url: str = (
        "https://lsst-archive.ampel.zeuthen.desy.de/api/lsst/archive/v1/"
    )
    ...
    def process(
        self,
        gen: Generator[TransientView, T3Send, None],
        t3s: None | T3Store = None,  # noqa: ARG002
    ) -> UBson | UnitResult:
        """
